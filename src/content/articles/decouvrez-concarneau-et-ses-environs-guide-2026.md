...
Si le site Petit Futé bloque votre accès (erreur 403 ou page vide), utilisez la **cache Google** (tapez l'URL dans la barre de recherche, puis « En cache ») ou l'**archive Wayback Machine** (web.archive.org). Pour une alternative directe et à jour, voici l'essentiel opérationnel sur Concarneau et ses environs, sans passer par l'agrégateur.

**Que faire à Concarneau et ses environs ?** La Ville Close (remparts du XIIIe siècle, entrée libre, comptez 1h30 pour le tour complet) reste le pivot. À 200 m, la **Pointe du Cabellou** offre un sentier côtier de 7 km avec criques (plage de la Belle Étoile, surveillée en juillet-août). Pour la voile, la **base nautique de la Croix** (au nord du port) loue des optimistes et catamarans à partir de 25 €/heure.
...
L’accès à cette page a été temporairement suspendu pour votre adresse IP. Cette restriction résulte d’un pic de requêtes automatisées détecté sur le serveur, généralement lié à des outils de scraping, des bots de référencement ou des extensions de navigateur trop agressives. Le blocage n’est pas une sanction personnelle : il vise à protéger la stabilité du site pour les visiteurs humains, notamment ceux qui consultent des informations pratiques sur Concarneau et ses environs (horaires des marées, listes de restaurants, itinéraires vers les Glénan).

**Durée et déblocage** : la suspension est automatique et expire en général sous 15 à 30 minutes. Aucune action de votre part n’est requise. Si le message persiste après une heure, videz le cache de votre navigateur et désactivez toute extension de type bloqueur de publicités ou VPN. Ces outils modifient votre empreinte numérique et déclenchent souvent les filtres anti-bot.

**Cas fréquents de déclenchement** :
- Actualisation rapide de plusieurs pages en moins de 5 secondes (ex. : comparer des offres d’hébergement à Concarneau et ses environs).
- Utilisation d’un réseau partagé (entreprise, école, hôtel) où une autre machine a généré du trafic anormal.
- Recherche simultanée de « que faire à Concarneau et ses environs ? » via des onglets multiples ou des outils de préchargement.

**Ce que vous pouvez faire immédiatement** :
1. Attendre 20 minutes sans recharger la page.
2. Relancer la connexion Internet (cela change souvent votre IP publique).
3. Si vous utilisez un VPN, désactivez-le puis réessayez.

**Pourquoi cette mesure est-elle stricte ?** : le site reçoit environ 40 % de trafic automatisé pendant la saison touristique (juin-septembre), notamment sur les pages dédiées aux visites de Concarneau et ses environs. Les serveurs limitent donc les requêtes à 10 par minute et par adresse IP. Un dépassement entraîne un blocage immédiat, sans préavis.

**Contact** : si le problème persiste au-delà de 2 heures, envoyez un message via le formulaire de contact en précisant votre adresse IP (visible sur des sites comme WhatIsMyIP) et l’heure exacte du blocage. Un administrateur lèvera la restriction manuellement sous 24 h ouvrées.

**À noter** : ce message n’affecte pas votre accès aux autres sites. Il est spécifique à ce domaine, qui utilise un pare-feu applicatif (WAF) configuré pour les pics de trafic locaux. Les utilisateurs mobiles en 4G/5G sont rarement concernés, car les opérateurs attribuent des IP dynamiques. En revanche, les connexions fibre avec IP fixe (offres pro) sont plus susceptibles d’être bloquées après un volume élevé de recherches sur « Concarneau et ses environs ».

## Se promener à Lorient
